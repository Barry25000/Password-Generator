...
function generatePassword() { //Start of password loop
  ...
  if (pLength >= 8 && pLength <= 128) { //check password length
    ...
    if (isUpperCase || isNumber || isLowerCase || isSpecial) {
      ...
      console.log(`this function has run ${indexer} times and will return ${outPutPassword}`)
      indexer++
      return outPutPassword;
    } else {
      console.log("Please pick at least 1 character set");
      console.log(`this function has run ${indexer} times and will return undefined`)
      indexer++
    ...
    return generatePassword();  //Re-run password loop
